  ...
  R6::R6Class(
    ...
    public = list(
      ...
      fit = function(X, y,
        ...
        {
          self$set_model(fit_regressor(x = self$X_train,
                                       y = self$y_train,
                                       method = self$method,
                                       ...))
        }
        ...
        if (self$pi_method %in% c("splitconformal", "kdesplitconformal"))
        {
          ...
          private$abs_calib_resids <- abs(y_calibration_sc - y_pred_calibration)
        }
      ...
      predict = function(X,
        ...
        if (is.null(level) && is.null(self$level))
        {
          ...
          if (self$pi_method %in% c("jackknifeplus", "kdejackknifeplus"))
          {
            ...
            {
              quantile_absolute_residuals <- quantile_scp(private$abs_calib_resids,
            ...
            if (identical(self$pi_method, "kdejackknifeplus"))
            {
              ...
              simulated_raw_calibrated_residuals <-
                rgaussiandens(private$calib_resids,
                  n = length(preds),
                  p = self$B,
                  seed = self$seed
                )
      ...
      summary = function(X,
        ...
        {
          res <- stats::t.test(x)
          return(c(as.numeric(res$estimate), 
