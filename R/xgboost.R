...
XgboostRegressor <- R6::R6Class(classname = "XgboostRegressor", inherit = learningmachine::BaseRegressor,
    public = list(initialize = function(name = "XgboostRegressor", type = "regression") {
        self$name <- name
        self$type <- type
    }, fit = function(X, y, ...) {
        if (is_package_available("xgboost") == FALSE) install.packages("xgboost",
            repos = c(CRAN = "https://cloud.r-project.org"))
        list_params <- list(...)
        stopifnot(!is.null(list_params$params))
        stopifnot(!is.null(list_params$params$objective))
        stopifnot(sum(grepl(list_params$params$objective, c("reg:squarederror", "reg:squaredlogerror",
...
XgboostClassifier <- R6::R6Class(classname = "XgboostClassifier", inherit = learningmachine::BaseClassifier,
    public = list(initialize = function(name = "XgboostClassifier", type = "classification") {
        self$name <- name
        self$type <- type
    }, fit = function(X, y, ...) {
        if (is_package_available("xgboost") == FALSE) install.packages("xgboost",
        ...
        stopifnot(!is.null(list_params$params))
        stopifnot(!is.null(list_params$params$objective))
        stopifnot(sum(grepl(list_params$params$objective, c("reg:logistic", "binary:logistic",
            "binary:logitraw", "binary:hinge", "multi:softmax", "multi:softprob"))) >=
            1)
        ...
        self$params <- list(...)
        self$set_model(fit_func_xgboost(x = self$X_train, y = self$y_train, ...))
        self$set_engine(list(fit = function(x, y) fit_func_xgboost(x, y, ...), predict = function(obj,
            X) {
