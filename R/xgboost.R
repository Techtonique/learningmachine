...
XgboostClassifier <-
  R6::R6Class(
    classname = "XgboostClassifier",
    inherit = learningmachine::BaseClassifier,
    public = list(
      initialize = function(name = "XgboostClassifier", type = "classification") {
        self$name <- name
        self$type <- type
      },
      fit = function(X, y, ...) {
        ...
        private$class_names <-
          as.character(levels(unique(as.factor(y))))
        y <- as.numeric(y) - 1
        self$X_train <- X
        self$y_train <- y
        self$params <- list(...)
