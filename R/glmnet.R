...
GlmnetRegressor <- R6::R6Class(
  classname = "GlmnetRegressor",
  ...
  public = list(
    ...
    fit = function(X, y, ...) {
      ...
      stopifnot(self$params$family %in% c("gaussian",
                                          ...
                                          "cox",
                                          "mgaussian"))
      self$set_model(glmnet::glmnet(x = self$X_train,
                                    y = self$y_train,
                                    ...))
      self$set_engine(list(
        fit = function(x, y) glmnet::glmnet(x, y, ...),
        predict = predict
      ))
...
GlmnetClassifier <- R6::R6Class(
  classname = "GlmnetClassifier",
  ...
  public = list(
    ...
    fit = function(X, y, ...) {
      ...
      self$params <- list(...)
      stopifnot(!is.null(self$params$family))
      stopifnot(self$params$family %in% c("binomial",
                                          "multinomial"))
      self$set_model(glmnet::glmnet(x = self$X_train, 
                                    y = self$y_train,
                                    ...))
