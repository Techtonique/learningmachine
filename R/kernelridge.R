                                        ...
                                        centering = FALSE, seed = 123, cl = NULL, ...)
{
  ...
  if(n > 500) # can use kmeans
  {
    ...
    if (with_kmeans == TRUE)
    {
      ...
      response_Y_clust <- sapply(1:centers, 
                                 function(i) apply(response_Y[which(cclust_obj$cluster == i)], 2, median))
      K <- matern32_kxx_cpp(x = X_clust, l = l)  
...
predict_matern32 <- function(fit_obj, newx, ci = NULL)
{
  ...
  if (fit_obj$centering) 
  {
    ...
    if (!is.null(fit_obj$with_kmeans))
    {
      ...
      #print("here1 -----")
      try_return <- try(return(drop(crossprod(K_star%*%fit_obj$coef)) + fit_obj$ym), 
                        silent = TRUE)
      ...
      #print("here3 -----")
      try_return <- try(return(drop(K_star%*%fit_obj$coef) + fit_obj$ym),
                        silent = TRUE)
      if (inherits(try_return, "try-error"))
      {
        #print("here4 -----")
