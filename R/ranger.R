...
RangerRegressor <- R6::R6Class(
  classname = "RangerRegressor",
...
RangerClassifier <- R6::R6Class(
  classname = "RangerClassifier",
  ...
  public = list(
    ...
    fit = function(X, y, ...) {
      ...
      self$set_engine(
        list(
          fit = function(x, y) fit_func_ranger_classification(x, y, ...),
          predict = function(obj, X)
            predict_func_ranger(obj, X, type = "response")
        )
      )
      return(base::invisible(self))
    },
...
predict_func_ranger <- function(obj, newx, ...)
{
  ...
  res <- try(predict(object = obj, data = newx, ...)$predictions, 
             silent = TRUE) # only accepts a named newx
  if (inherits(res, "try-error"))
  {
    res <- try(predict(object = obj, data = matrix(newx, nrow=1), 
                       ...)$predictions, 
               silent = TRUE)
  }
  return(res)
