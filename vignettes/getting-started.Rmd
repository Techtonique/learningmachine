...
```{r fig.width=7.2}
obj$set_level(95L)
res <- obj$predict(X = X_test, method = "splitconformal")
...
```{r fig.width=7.2}
res <- obj$predict(X = X_test,  method = "jackknifeplus")
...
```{r}
obj$predict_proba(X_test, level=95)
```

```{r}
mean(obj$predict(X_test, level=95) == y_test)
```

```{r}
caret::confusionMatrix(data = obj$predict(X_test, level=95), reference = y_test)
```
...
# 2 - 5 `BaseClassifier` classification
...
t0 <- proc.time()[3]
mean(obj$predict(X_test, level=95, 
                 method = "kdesplitconformal") == y_test)
cat("Elapsed: ", proc.time()[3] - t0, "s \n")
```

```{r}
t0 <- proc.time()[3]
caret::confusionMatrix(data = obj$predict(X_test, 
                                          method = "kdesplitconformal"), reference = y_test)
cat("Elapsed: ", proc.time()[3] - t0, "s \n")
```
